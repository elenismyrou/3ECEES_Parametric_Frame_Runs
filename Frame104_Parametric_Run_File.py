...
# Version check : v4.0 / 01.01.2022 - Parametric
# ------------------------------------------------------------------------
...
f_ro=[0.8, 1.0]  # no ref
f_concrete=[0.49, 1.00, 1.51] # lower bound and upper bound from our study IUSS Bal et al., 2008
...
f_column_depth=[0.87, 1.00, 1.13]  # from Bal et al. 2008 IUSS CoV 0.13 for PFN frames

current_analysis=0

# Read the acceleration files (in txt format, single data column)
# In this example, the data are coming from AFAD / Turkey
# The ground motion files are in .asc format
# Ass .asc files were already turned into single-column txt files
# and for removing the header lines, for use in OpenSees.


# Now read all the txt files transformed from the asc files, and run as many analyses
filelist=osys.listdir("GMfile/")

# Find the number of the txt files for feeding into OpenSees
txt_no=0
for filename in osys.listdir("GMfile/"):
    if filename.endswith(".txt"):
        txt_no+=1

number_of_analysis=txt_no*len(f_ro)*len(f_concrete)*len(f_steel)*len(f_span)*len(f_ground_height)*len(f_column_depth)
                    ...
                    for FSP in f_span:
                        for FGH in f_ground_height:
                            for FUH in f_column_depth:
                                ...
                                # 1 : concrete strength
                                # 2 : reinforcing steel strength
                                # 3 : span lengths
                                # 4 : ground floor height
                                # 5 : column section depth (m)
                                ...
                                with open('Out/' + filename2[:-4] + '_Analysis' + str(current_analysis) + '.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
